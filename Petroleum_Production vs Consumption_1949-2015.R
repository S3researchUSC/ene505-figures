# Petroleum_Production vs Consumption_1949-2015 #
...
# only keep natural gas production (dry and liquid) ------
  dt_prod = dt_prod[ Description %like% "Crude Oil", c(2:3), with = FALSE ]

# only keep natural gas consumption ------
  dt_cons = dt_cons[ Description %like% "Petroleum", c(2:3), with = FALSE ]
...
# ---------------------------------------------------------------
  ...
  line_petro_annual_prod_vs_cons = ggplot(dt_annual, aes(x = year, y = value, linetype = type)) + 
    geom_line(stat = "identity", color = "gray10", size = 1) +
    labs(title = "1949 - 2014 Annual U.S. Petroleum Production and Consumption",
  ...
  ggsave(line_petro_annual_prod_vs_cons, 
         filename = "Petroleum_Production vs Consumption_1949-2014_Annual_LTS.png", 
         width = 11.1, 
         height = 6.25, 
         dpi = 400)
  
  line_petro_monthly_prod_vs_cons = ggplot(dt_monthly, aes(x = date, y = value, linetype = type)) + 
    geom_line(stat = "identity", color = "gray10") +
    labs(title = "1973 - 2014 Monthly U.S. Petroleum Production and Consumption",
         subtitle = "Data: EIA Annual Energy Review", 
         x = NULL,
         y = "Quadrillion BTU",
         linetype = NULL) +
  ...
  ggsave(line_petro_monthly_prod_vs_cons, 
         filename = "Petroleum_Production vs Consumption_1973-2014_Monthly_LTS.png", 
         width = 11.1, 
         height = 6.25, 
         dpi = 400)
  ...
  wsize = 3
  csize = 2
  tlab = "1949 - 2014 Annual U.S. Petroleum Production and Consumption"
  ...
  seg_petro_prod_vs_cons_1949_2015 = f.segplot(dt, xval, yval, fillval, wsize, csize, 
                                            tlab, sublab, xlab, ylab, leglab, gval, leg.ord, plot.cols) + 
    scale_x_continuous(breaks = seq(1945,2015,5), expand = c(0,0)) +
    scale_y_continuous(breaks = seq(0,45,5), expand = c(0,0), limits = c(0, 45)) +
    theme(legend.position = "bottom")
  
  ggsave(seg_petro_prod_vs_cons_1949_2015, 
         filename = "Petroleum_Production vs Consumption_1949-2014_Annual_STS.png", 
         width = 11.1, 
         height = 6.25, 
         dpi = 400)
