...
# ------------------------------------ figures ------------------------------------
  ...
  # line, reserves -----
    ...
    labs_line_res = ng_agg[type == 'reserves' & year == 2017]
    setorder(labs_line_res, 'value')
    labs_line_res[, position := value]
    labs_line_res[1:2, position := c(290,325)]
    
    line_res = ggplot(ng_agg[type == 'reserves'], aes(x = year, y = value, color = label)) + 
      geom_line(size = 0.9) +
      labs(title = 'Annual gross natural gas reserves by country (1980-2017)',
           subtitle = 'Trillion cubic feet', 
           caption = 'Countries with top 5 highest reserves in 2017 shown individually. All other countries aggregated. Data: U.S. Energy Information Administration', 
           x = NULL,
           y = NULL) +
      guides(color = 'none') +
      scale_x_continuous(breaks = seq(1980,2020,5), limits = c(1980,2017), expand = c(0,0)) +
      scale_y_continuous(expand = c(0,0), breaks = seq(0,3e3,5e2), labels = scales::comma) +
      scale_color_manual(values = pal_res) + 
      theme_line +
      geom_text(data = labs_line_res, aes(x = Inf, y = position, label = paste0(' ', label), color = label), hjust = 0,
                size = 6.0, fontface = 'plain', family = 'Secca Soft')
    
    line_res = ggplotGrob(line_res)
    line_res$layout$clip[line_res$layout$name == "panel"] = "off"
    
    ggsave(line_res, 
           filename = here::here('figures', 'world', 'natural-gas-reserves-by-country_annual_1980-2017_lts.pdf'), 
           width = 11.5, 
           height = 6.25)
    
    embed_fonts(here::here('figures', 'world', 'natural-gas-reserves-by-country_annual_1980-2017_lts.pdf'),
                outfile = here::here('figures', 'world', 'natural-gas-reserves-by-country_annual_1980-2017_lts.pdf'))
    
  # area, reserves (absolute) -----
    
    labs_area_res = ng_agg[type == 'reserves' & year == 2017][order(factor(label, levels = rev(c('All Other Countries', rev(top5_res)))))]
    labs_area_res[, cum_sum := cumsum(value)] 
    labs_area_res[, difference := diff(c(0,cum_sum))/2]
    labs_area_res[, position := cum_sum - difference]
    
    area_res = ggplot(ng_agg[type == 'reserves'], 
                      aes(x = year, y = value, fill = factor(label, levels = c('All Other Countries', rev(top5_res))))) + 
      geom_area() +
      labs(title = 'Annual gross natural gas reserves by country (1980-2017)',
           subtitle = 'Trillion cubic feet', 
           caption = 'Countries with top 5 highest reserves in 2017 shown individually. All other countries aggregated. Data: U.S. Energy Information Administration', 
           ...
           y = NULL) +
      scale_x_continuous(breaks = seq(1980,2020,5), limits = c(1980,2017), expand = c(0,0)) +
      scale_y_continuous(expand = c(0,0), breaks = seq(0,7e3,1e3), limits = c(0,7e3), labels = scales::comma) +
      guides(fill = 'none',
             color = 'none') +
      scale_color_manual(values = pal_res) + 
      scale_fill_manual(values = pal_res) + 
      theme_area_labeled +
      geom_text(data = labs_area_res, aes(x = Inf, y = position, label = paste0(' ', label), color = label), 
                hjust = 0, size = 6.0, fontface = 'plain', family = 'Secca Soft')
    
    area_res = ggplotGrob(area_res)
    area_res$layout$clip[area_res$layout$name == "panel"] = "off"
    
    ggsave(area_res, 
           filename = here::here('figures', 'world', 'natural-gas-reserves-by-country_annual_1980-2017_ats_absolute.pdf'), 
           width = 11.5, 
           height = 6.25)
    
    embed_fonts(here::here('figures', 'world', 'natural-gas-reserves-by-country_annual_1980-2017_ats_absolute.pdf'),
                outfile = here::here('figures', 'world', 'natural-gas-reserves-by-country_annual_1980-2017_ats_absolute.pdf'))
    
    
  # area, reserves (proportion) -----
    
    labs_area_res_prop = ng_agg[type == 'reserves' & year == 2017][order(factor(label, levels = rev(c('All Other Countries', rev(top5_res)))))]
    labs_area_res_prop[, cum_sum := cumsum(prop)] 
    labs_area_res_prop[, difference := diff(c(0,cum_sum))/2]
    labs_area_res_prop[, position := cum_sum - difference]
    
    area_res_prop = ggplot(ng_agg[type == 'reserves'], 
                           aes(x = year, y = prop, fill = factor(label, levels = c('All Other Countries', rev(top5_res))))) + 
      geom_area() +
      labs(title = 'Annual gross natural gas reserves by country (1980-2017)',
           subtitle = 'Share of global gross natural gas reserves', 
           caption = 'Countries with top 5 highest reserves in 2017 shown individually. All other countries aggregated. Data: U.S. Energy Information Administration', 
           x = NULL,
           y = NULL) +
      scale_x_continuous(breaks = seq(1980,2020,5), limits = c(1980,2017), expand = c(0,0)) +
      ...
      scale_fill_manual(values = pal_res) + 
      theme_area_labeled +
      geom_text(data = labs_area_res_prop, aes(x = Inf, y = position, label = paste0(' ', label), color = label), 
                hjust = 0, size = 6.0, fontface = 'plain', family = 'Secca Soft')
    
    area_res_prop = ggplotGrob(area_res_prop)
    area_res_prop$layout$clip[area_res_prop$layout$name == "panel"] = "off"
    
    ggsave(area_res_prop, 
           filename = here::here('figures', 'world', 'natural-gas-reserves-by-country_annual_1980-2020_ats_proportion.pdf'), 
           width = 11.5, 
           height = 6.25)
    
    embed_fonts(here::here('figures', 'world', 'natural-gas-reserves-by-country_annual_1980-2020_ats_proportion.pdf'),
                outfile = here::here('figures', 'world', 'natural-gas-reserves-by-country_annual_1980-2020_ats_proportion.pdf'))
