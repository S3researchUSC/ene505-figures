# taken from: http://ellisp.github.io/blog/2016/08/18/dualaxes
# and edited to change aesthetics

dualplot <- function(x1, y1, y2, xbreaks, y1breaks, y2breaks, col, 
                     x2 = x1, 
  ...
  mtext(paste0("\n", ylab1, "\n", "\n"), side = 2, col = col[1], line = 1.5, cex=1.3)
  ...
  plot(x2, y2,   xlab="", ylab="", axes = FALSE, type = "l", lwd = lwd[2],
       col = col[2], xlim = range(xbreaks), ylim = c(min(y2breaks), max(y2breaks)),
       xaxs="i", yaxs="i")

  ## add second vertical axis (on right) and its label
  mtext(paste0("\n", ylab2, "\n"), side = 4, col = col[2], line = 4.5, cex=1.3)
